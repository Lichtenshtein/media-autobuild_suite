...
if [[ $ffmpeg != no ]] && enabled whisper &&
    ...
    extracommands+=(-DGGML_VULKAN=ON -DBUILD_SHARED_LIBS=OFF -DWHISPER_BUILD_TESTS=OFF -DWHISPER_BUILD_EXAMPLES=OFF -DWHISPER_BUILD_SERVER=OFF -DWHISPER_USE_SYSTEM_GGML=OFF -DGGML_CCACHE=OFF -DGGML_OPENCL=ON -DGGML_SSE42=ON -DGGML_AVX=ON -DGGML_F16C=ON -DGGML_AVX2=ON -DGGML_BMI2=ON -DGGML_FMA=ON)
    do_cmakeinstall "${extracommands[@]}"
